...
## Current icon — supplied by the author

The author supplied the final icon on 2026-09-08. `AppIcon.png` is that provided PNG, copied without creative edits. `scripts/icon.sh` produces standard ICNS sizes. Info.plist references AppIcon, and the build includes it in the .app bundle for Finder. The generated versions and prompts below are historical references.

## Simplified revision — 2026-09-08

The simplified revision replaced the initial glossy version at the author's request. Edited with built-in ImageGen, with a second pass to produce genuine alpha transparency (verified with `sips`). The icon retains the tray/file mark with matte surfaces and shallow depth.
...
The tool returned a 1254×1254 PNG; `icon.sh` resamples it to standard macOS icon sizes. The source and bundled icon are included under the project's Apache-2.0 license to the extent applicable.
